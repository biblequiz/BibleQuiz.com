...
export class DataTypeHelpers {
    ...
    public static formatAddress(address: Address | null): string | null {
        ...
        if (
            !DataTypeHelpers.isNullOrEmpty(address.State) ||
            null != address.ZipCode
        ) {
            const stateZip: string = (
                DataTypeHelpers.formatNullableString(address.State) +
                " " +
                DataTypeHelpers.formatZipCode(address.ZipCode)
            ).trim();
    ...
    public static formatDateRange(
        ...
        const startDateRange: Date | null =
            typeof startDate == "string"
                ? this.parseDateOnly(startDate)
                : startDate;
        const endDateRange: Date | null =
            typeof endDate == "string" ? this.parseDateOnly(endDate) : endDate;
     ...
     */
    public static hasEnumFlag<T extends number>(
        flagsValue: T,
        individualValue: T,
    ): boolean {
     ...
     */
    public static formatTimeSpanAsTime(
     ...
     * @returns A string for time.
     */
    public static formatAsTime(hours: number, minutes: number): string {
    ...
    public static parseTimeSpan(
        ...
        if (value!.includes(".")) {
            ...
            if (
                !isNaN(parseInt(potentialDays)) &&
                value!.substring(dotIndex + 1).includes(":")
            ) {
     ...
     */
    public static parseTimeSpanAsMinutes(
        value: string | null | undefined,
    ): number | null {
    ...
    /**
     * Adds a suffix to an ordinal number (e.g., 1st, 2nd, 3rd, 4th).
     * @param number Number to convert.
     * @returns The number with its ordinal suffix.
     */
    public static ordinalWithSuffix(number: number): string {
